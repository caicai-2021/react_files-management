...
export default class Intro extends Component {
    ...
    componentDidMount() {
        ...
        async function getdata() {
            const result = await reqTyper();
            const data = [result[0].管理员, result[0].用户, result[0].待审核, result[0].已离开];
            console.log(data)
            return data;
        };
        const Data = getdata();
    ...
    render() {
        return (
            <div style={{ width: '100%', height: '100%' }}>
                ...
                <div id="main_1" style={{ width: '100%', height: '300px' }}>caci</div>
            </div>
        )
    }
}
